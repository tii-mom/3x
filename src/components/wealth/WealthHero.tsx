...
export const WealthHero: React.FC<WealthHeroProps> = ({
  ...
  const pnl = portfolio.investmentPnL !== undefined ? portfolio.investmentPnL : portfolio.pnlSinceStart;
  const netCap = portfolio.netContributions || portfolio.startingCapital || 1;
  const pnlPercent = (pnl / netCap) * 100;
    ...
    <div className={`relative overflow-hidden rounded-3xl bg-white border border-[#E2E7F0] p-6 shadow-sm ${className}`}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="text-[12px] font-bold text-[#64748B] tracking-wider uppercase font-mono">
            MY AI ASSETS
          </span>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[13px] font-bold text-[#11141C]">
              {formatUsd(portfolio.nextMilestone, 0, 0)} Target
            </div>
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="text-[11px] text-[#64748B]">
            {formatUsd(Math.max(0, portfolio.nextMilestone - portfolio.nav))} to go
          </div>
        </div>
      </div>
    </div>
